...
function bind(){
  ...
  document.addEventListener('keydown',e=>{if(!$('calculatorSheet').classList.contains('open')){if(e.key==='Escape')closeSheet();return}if(e.key==='Escape'){closeCalculator();return}if(e.key==='Enter'||e.key==='='){e.preventDefault();applyCalculation();return}const map={Backspace:'backspace',Delete:'clear',',':'.'};const key=map[e.key]||e.key;if(/^[0-9.+\-*/%()]$/.test(key)){e.preventDefault();handleCalcKey(key)}});
